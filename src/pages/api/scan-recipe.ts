...
export const POST: APIRoute = async ({ request, locals }) => {
	try {
		...
		// Default prep/cook times if extracted from HelloFresh/HomeChef card
		...
		// Default structured ingredients list if not supplied
		...
		// PortableText block structure for instructions
		const rawInstructions = customInstructions.trim() || 
			"1. Prep Ingredients: Wash and dry all produce. Chop vegetables into 1/2-inch pieces.\n2. Sear & Roast: Heat oil in pan over medium-high heat. Sear protein until golden brown, then transfer to grill or oven to complete cooking.\n3. Serve & Garnish: Drizzle with sauce, squeeze fresh lemon, and serve warm.";
		...
		// Default featured image per source/equipment
		...
		const featuredImageObj = JSON.stringify({
			id: `media-card-${Date.now()}`,
			src: imageUrl,
			alt: title,
		});

		// Insert into Cloudflare D1 database via locals.runtime.env.DB
		// @ts-ignore
		const db = locals?.runtime?.env?.DB;

		if (db) {
			await db.prepare(`
				INSERT INTO ec_recipes (id, slug, status, title, recipe_source, equipment, cooking_method, prep_time, cook_time, servings, ingredients, allergens, cottage_license_notice, excerpt, featured_image, content, published_at)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
			`).bind(
				recipeId,
				slug,
				...
				defaultIngredients,
				allergens,
				source.includes("Cottage") ? "Baked in a Cottage Food Operation (License #CFO-LA-2026-9481)." : `Scanned from ${source} Recipe Card.`,
				`Extracted recipe for ${title} from ${source} card photo.`,
				featuredImageObj,
